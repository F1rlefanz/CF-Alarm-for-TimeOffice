...
class CalendarContentObserver(
    ...
    companion object {
        ...
        private const val EXECUTION_PROBABILITY =
            0.7f  // 70% execution rate for battery optimization
    ...
    override fun onChange(selfChange: Boolean, uri: Uri?) {
        ...
        Logger.d(
            LogTags.MAINTENANCE_L3,
            "⚡ LEVEL 3: Calendar change detected - URI: $uri, SelfChange: $selfChange, PendingChanges: $pendingChanges"
        )
        ...
        debounceJob = observerScope.launch {
            try {
                ...
                if (System.currentTimeMillis() - lastChangeTime >= DEBOUNCE_DELAY_MS) {
                    ...
                    Logger.d(
                        LogTags.MAINTENANCE_L3,
                        "⚡ LEVEL 3: More recent changes detected, extending debounce"
                    )
                }
                ...
                Logger.e(
                    LogTags.MAINTENANCE_L3,
                    "❌ LEVEL 3: Error in debounced change processing",
                    e
                )
            }
        }
    ...
    private suspend fun performReactiveAlarmMaintenance(uri: Uri?, changeCount: Int) {
        ...
        if (Random.nextFloat() > EXECUTION_PROBABILITY) {
            Logger.d(
                LogTags.MAINTENANCE_L3,
                "🎲 LEVEL 3: Skipping reactive check (probabilistic battery optimization)"
            )
            return
        }
        ...
        try {
            ...
            if (futureAlarms.size >= MINIMUM_FUTURE_ALARMS_L3 && changeCount <= 3) {
                Logger.d(
                    LogTags.MAINTENANCE_L3,
                    "✅ LEVEL 3: Sufficient alarms for minor changes (${futureAlarms.size} >= $MINIMUM_FUTURE_ALARMS_L3)"
                )
                return
            }
            ...
            if (selectedCalendarIds.isEmpty()) {
                Logger.w(
                    LogTags.MAINTENANCE_L3,
                    "⚠️ LEVEL 3: No calendars selected, skipping reactive maintenance"
                )
                return
            }
            ...
            Logger.d(
                LogTags.MAINTENANCE_L3,
                "🔍 LEVEL 3 REACTIVE SCAN: $REACTIVE_LOOKAHEAD_DAYS days ahead for ${selectedCalendarIds.size} calendars"
            )
            ...
            if (reactiveEventsResult.isFailure) {
                Logger.w(
                    LogTags.MAINTENANCE_L3,
                    "❌ LEVEL 3: Failed to get reactive calendar events",
                    reactiveEventsResult.exceptionOrNull()
                )
                return
            }
            ...
            Logger.business(
                LogTags.MAINTENANCE_L3,
                "📅 LEVEL 3 SCAN: Found ${reactiveEvents.size} events in reactive range"
            )
            ...
            if (newShiftMatches.isEmpty()) {
                ...
                val orphanedAlarms = futureAlarms.filter { alarm ->
                    ...
                    !reactiveEvents.any { event ->
                        val eventAlarmTime =
                            shiftRecognitionEngine.getAllMatchingShifts(listOf(event))
                                .firstOrNull()?.calculatedAlarmTime
                                ?.atZone(ZoneId.systemDefault())?.toInstant()?.toEpochMilli()
                        eventAlarmTime?.let { abs(it - alarm.triggerTime) < 60 * 1000 } ?: false
                    }
                }
                ...
                if (orphanedAlarms.isNotEmpty()) {
                    ...
                    for (orphanedAlarm in orphanedAlarms) {
                        try {
                            ...
                            Logger.d(
                                LogTags.MAINTENANCE_L3,
                                "✅ LEVEL 3: Cleaned up orphaned alarm: ${orphanedAlarm.shiftName}"
                            )
                        } catch (e: Exception) {
                            Logger.e(
                                LogTags.MAINTENANCE_L3,
                                "❌ LEVEL 3: Failed to clean up orphaned alarm: ${orphanedAlarm.shiftName}",
                                e
                            )
                        }
                    }
                    ...
                    Logger.d(
                        LogTags.MAINTENANCE_L3,
                        "💡 LEVEL 3: No new shifts or orphaned alarms - calendar changes were non-alarm-relevant"
                    )
                }
                return
            }
            ...
            if (shiftConfig == null || !shiftConfig.autoAlarmEnabled) {
                Logger.d(
                    LogTags.MAINTENANCE_L3,
                    "⚠️ LEVEL 3: Auto-alarm disabled, skipping reactive alarm creation"
                )
                return
            }
            ...
            if (createResult.isSuccess) {
                ...
                for (newAlarm in createdAlarms) {
                    try {
                        ...
                        Logger.d(
                            LogTags.MAINTENANCE_L3,
                            "✅ LEVEL 3: Reactive system alarm scheduled: ${newAlarm.shiftName}"
                        )
                    } catch (e: Exception) {
                        Logger.e(
                            LogTags.MAINTENANCE_L3,
                            "❌ LEVEL 3: Failed to schedule reactive alarm: ${newAlarm.shiftName}",
                            e
                        )
                    }
                }
                ...
                Logger.w(
                    LogTags.MAINTENANCE_L3,
                    "❌ LEVEL 3: Failed to create reactive alarms",
            ...
            Logger.e(
                LogTags.MAINTENANCE_L3,
                "❌ LEVEL 3: Critical error during reactive maintenance (other levels still active)",
                e
            )
        }
    }
    ...
    fun getObserverStats(): String {
        return buildString {
            ...
            appendLine(
                "Last change: ${
                    if (lastChangeTime > 0) java.time.Instant.ofEpochMilli(
                        lastChangeTime
                    ) else "Never"
                }"
            )
