...
- ✅ **P1 · Ladefehler der Kern-Pipeline werden nicht kommuniziert** — behoben in `d3f95e9`: `SnackbarHost` im `MainContentScreen`; `calendarState`/`shiftState`/`alarmState.error` werden per `LaunchedEffect` als Snackbar gezeigt (Kalender mit „Wiederholen"-Action) und geleert. (Der auskommentierte Block in `EventListScreen` ist damit obsolet — optionaler Cleanup.)
